...
```bash
uv run .Codex/skills/pageindex-search/scripts/pageindex_search.py <document-path> "<question>"
```
...
uv run .Codex/skills/pageindex-search/scripts/pageindex_search.py <document-path> "<question>" --no-answer

# Tree retrieval + LLM answer synthesis
uv run .Codex/skills/pageindex-search/scripts/pageindex_search.py <document-path> "<question>" --answer-mode llm

# Tree retrieval + LLM rerank + LLM answer
uv run .Codex/skills/pageindex-search/scripts/pageindex_search.py <document-path> "<question>" --llm-rerank --answer-mode llm

# Same call, with inline LLM quality evaluation in the same run
uv run .Codex/skills/pageindex-search/scripts/pageindex_search.py <document-path> "<question>" --llm-rerank --answer-mode llm --eval-inline

# Pure LLM retrieval + LLM answer
uv run .Codex/skills/pageindex-search/scripts/pageindex_search.py <document-path> "<question>" --retrieval-mode llm --answer-mode llm

# Build/resolve index only
uv run .Codex/skills/pageindex-search/scripts/pageindex_search.py <document-path> --index-only

# Open interactive conversation REPL over the index
uv run .Codex/skills/pageindex-search/scripts/pageindex_search.py <document-path>
```

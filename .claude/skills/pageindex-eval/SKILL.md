...
uv run .claude/skills/pageindex-eval/scripts/vectorless_rag_eval.py <document-path> "<question>"

# Retrieval-only mode (no answer synthesis)
uv run .claude/skills/pageindex-eval/scripts/vectorless_rag_eval.py <document-path> "<question>" --no-answer

# Build or resolve index only
uv run .claude/skills/pageindex-eval/scripts/vectorless_rag_eval.py <document-path> --index-only

# Interactive REPL after indexing/resolving
uv run .claude/skills/pageindex-eval/scripts/vectorless_rag_eval.py <document-path>
```
...
- `.claude/skills/pageindex-eval/scripts/vectorless_rag_eval.py`: primary uv script for provided documents.
